...
function noteOn(note, velocity) {
    ...
    /*   Old version of filter Attack and Decay, replaced by the if-else below
    ...
    filter.frequency.exponentialRampToValueAtTime(cutoffValue + 20, ctx.currentTime + filterAttackTime); // Ramp up della frequenza
    ...
    */
    ...
    filterSustainKnob.addEventListener("input", () => {
        const filterSustainValue = parseFloat(filterSustainKnob.value);
        Object.keys(oscillators).forEach((key) => {
            const oscData = oscillators[key];
            if (oscData && oscData.filter) {
                oscData.filter.frequency.setTargetAtTime(filterSustainValue * 20000, ctx.currentTime, 0.1);
                console.log(`Filter Sustain aggiornato: ${filterSustainValue * 20000} Hz`);
            }
        });
    }); 
